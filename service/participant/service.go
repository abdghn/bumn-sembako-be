...
type Service interface {
	...
	CheckId(criteria map[string]interface{}) int64
...
func (s *service) CheckId(criteria map[string]interface{}) int64 {
	var result int64
	query := s.db.Table("participants").Select(" count(nik)").Where(criteria).Where("deleted_at IS NULL")
